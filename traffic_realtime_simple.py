...
Hiển thị real-time trên Streamlit - KHÔNG GHI FILE
"""
...
st.markdown("### YOLO + ByteTrack - Real-time Processing")
st.markdown("---")
...
st.caption("🚀 YOLO + ByteTrack + OpenCV | Xử lý real-time + Lưu video output")
            ...
            y_pos = 25
            cv2.putText(frame, f"FPS: {current_fps:.1f}", (10, y_pos),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
            y_pos +=WRITE TO OUTPUT VIDEO =====
            out.write(frame)
            
            # =====  25
            cv2.putText(frame, f"Frame: {frame_idx}/{total_frames}", (10, y_pos),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            y_pos += 20
            cv2.putText(frame, f"Vehicles: {len(breakdown)}", (10, y_pos),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            y_pos += 20
            cv2.putText(frame, f"Passed: {vehicle_count}", (10, y_pos),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
            ...
            # Update progress
            progress = frame_idx / total_frames
            progress_placeholder.progress(progress, text=f"Đang xử lý: {progress*100:.1f}%")
        ...
        if not st.session_state.stop_requested:
            ...
            st.info(f"📹 Video đã lưu: {output_path}")
            
            # Get file size
            file_size_mb = os.path.getsize(output_path) / (1024 * 1024)
            st.info(f"💾 Kích thước: {file_size_mb:.2f} MB")
...
if st.session_state.processing_complete and st.session_state.output_video_path:
    st.markdown("---")
    st.markdown("## 🎬 XEM LẠI VIDEO ĐÃ XỬ LÝ")
    
    if os.path.exists(st.session_state.output_video_path):
        st.success(f"✅ Video: `{st.session_state.output_video_path}`")
...
# ==================== FOOTER ====================
st.markdown("---")
st.caption("🚀 YOLO + ByteTrack + OpenCV | Xử lý trực tiếp - Không ghi file")
